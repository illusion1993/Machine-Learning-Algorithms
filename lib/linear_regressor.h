...
double LinearRegressor::trainingHypothesisValue(int sample_number) {
	/*
	...
	*/
	...
	for (int feature_number = 0; feature_number <= _training_dataset -> depth(); feature_number++) {
		hypothesis += (_parameters[feature_number] * _training_dataset -> getInput(sample_number, feature_number));
	}
	return hypothesis;
}
...
void LinearRegressor::gradientDescent(double learning_rate, double allowed_rms_error) {
	...
	while (previous_rms_error > allowed_rms_error) {
		...
		for (int sample_number = 0; sample_number < _training_dataset -> size(); sample_number++) {
			hypothesis_error[sample_number] = trainingHypothesisValue(sample_number) - _training_dataset -> getOutput(sample_number);
...
void LinearRegressor::printResults() {
	...
	for (int i = 0; i < _training_dataset -> size(); i++) {
		...
		cout << ": " << _training_dataset -> getOutput(i) << ", h(X) = " << trainingHypothesisValue(i) << "\n";
	}
}